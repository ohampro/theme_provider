...
abstract class AppTheme<T> {
  ...
  T of(int mode) {
    ...
    if (mode == ThemeService.systemMode) {
      ...
      mode = brightness == Brightness.light ? ThemeService.lightMode : ThemeService.darkMode;
    }
  ...
  String nameOf(int mode) => 
    mode == ThemeService.systemMode ? 'System' : 
    mode == ThemeService.lightMode ? 'Light' : 
    mode == ThemeService.darkMode ? 'Dark' : 
    mode.toString();
